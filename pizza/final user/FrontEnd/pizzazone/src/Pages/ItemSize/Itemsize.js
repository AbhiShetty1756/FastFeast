...
export default function Itemsize() {
    ...
    const getItemSize = () => {
        ...
        const url = `http://localhost:7070/itemSize/${itemid}/${itemsize}`
    ...
    const getToppings = () => {
        const url = `http://localhost:7070/toppings/showAll`
        axios.get(url).then((response) => {
            const result = response.data
            console.log(result)
            if (result.status === 'success') {
    ...
    //     const url = `http://localhost:7070/itemSize/${itemid}/${change}`
    //     axios.get(url).then((response) => {
    //         const result = response.data
    //         console.log(result)
    //         if (result.status === "success") {
    ...
    const add2db = (varioussize) => {
        ...
        if (toppingId === '') {
            const url = `http://localhost:7070/cart/addWithoutToppings`
            ...
            const url = `http://localhost:7070/cart/addWithToppings`
